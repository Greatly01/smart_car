...
def update_models(event):
    ...
    # 模型名称和路径配置
    model_configs = [
        {"name_prefix": "fruit_board_", "path": f"/home/demo/.gazebo/models/{random.choice(fruit_board_folders)}/model.sdf"},
        {"name_prefix": "vage_board_", "path": f"/home/demo/.gazebo/models/{random.choice(vege_board_folders)}/model.sdf"},
        {"name_prefix": "dessert_board_", "path": f"/home/demo/.gazebo/models/{random.choice(dessert_board_folders)}/model.sdf"}
    ]
    ...
    for room in rooms:
        for model in models:
            model_name = f"{model['name_prefix']}{room['name']}"
...
if __name__ == "__main__":
    ...
    # 设置更新间隔，单位为秒
    update_interval = 20
    rospy.Timer(rospy.Duration(update_interval), update_models)
    rospy.spin()
