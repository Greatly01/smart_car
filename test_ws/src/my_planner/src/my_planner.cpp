...
namespace my_planner
{
    ...
    bool MyPlanner::computeVelocityCommands(geometry_msgs::Twist& cmd_vel)
    {
        ...
        if(pose_adjusting_ == true)
        {
            ...
            cmd_vel.angular.z = final_yaw * 4.0;
        ...
        for(int i= target_index_;i<global_plan_.size();i++)
        {
            ...
            {
                target_pose = pose_base;
                ROS_WARN("所有路径点都被选中，目标已到达");
                // ROS_INFO("任务完成");
            }
        }
        cmd_vel.linear.x = target_pose.pose.position.x * 5.0;//修改前进速度
        cmd_vel.linear.y = target_pose.pose.position.y * 3.0;//修改平移速度
        ...
        // 添加调试输出
        ROS_INFO("PID调试信息: 误差 = %.4f, 积分项 = %.4f, 微分项 = %.4f, 输出 = %.4f", angular_error, error_sum, error_diff, output);
...
//                 target_pose = pose_base; 
//         }
//         cmd_vel.linear.x = target_pose.pose.position.x * 3.5;
//         cmd_vel.angular.z = target_pose.pose.position.y * 5.0;
