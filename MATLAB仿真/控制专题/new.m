...
%% 独立动画控制参数输入
% 第一部分：Fuzzy-PP动画
skip_part1 = input('是否跳过 Fuzzy-PP 动画？(1=是, 0=否): ');
speed_part1 = input('Fuzzy-PP 动画速度 (1=正常, >1加速, <1减速): ');

% 第二部分：PUMA560动画
skip_part2 = input('是否跳过 PUMA560 动画？(1=是, 0=否): ');
%base_height = input('设置机械臂基座高度 (m, 推荐0.3-0.8): ');
speed_part2 = input('PUMA560 动画速度 (1=正常, >1加速, <1减速): ');
...
if ~isfile(fisFile)
    ...
    % 规则表
    ruleList = [...
        1 1 3 1 1; 1 2 3 1 1; 1 3 2 1 1;
        2 1 3 1 1; 2 2 2 1 1; 2 3 1 1 1;
        3 1 2 1 1; 3 2 1 1 1; 3 3 1 1 1];
...
for k = 1:N1
    ...
    if ~skip_part1
        ...
        if mod(k,refresh_step)==0
            ...
            drawnow limitrate; 
            pause(dt1*slow_factor / speed_part1);
        end
    end
end
...
% 定义末端执行器Z坐标
z_tcp = 0.75;  % 添加这一行，设置末端执行器高度

% 1. 轨迹适配与基座设置
reach_rad = 0.65; 
xL1 = x_ref1; yL1 = y_ref1;  % 使用Part1生成的轨迹
...
base_height=0.55;
robot.base = transl(xc, yc, base_height);  % 应用用户输入高度
R_tcp = trotx(pi/2)*trotz(-pi/2);  % 末端姿态修正

% 3. 逆运动学求解
qs = zeros(N1,6); 
% 使用标称姿态或随机姿态替代q0
try
    q_prev = robot.qn;  % 使用标称姿态
catch
    q_prev = robot.random;  % 如qn不可用，则使用随机姿态
end

for i = 1:N1
    Tgoal = transl(xs_shift(i), ys_shift(i), z_tcp) * R_tcp;
    sol = robot.ikine6s(Tgoal, 'ru');  % 鲁棒逆解
...
% 5. 动画参数设置（带速度控制）
refresh_step_robot = 5;  
animation_interval = dt1 * refresh_step_robot / speed_part2;  % 计算动画间隔
...
if ~skip_part2
    ...
    % 速度控制动画循环
    tic; last_time = 0;
    for i = 1:refresh_step_robot:N1
        while toc < last_time + animation_interval
            pause(0.001);
        end
        last_time = toc;
...
% 计算5s内对应的索引
time_limit = 5; % 时间限制为5s
index_limit = round(time_limit / dt1);

% 7. 全面运动评估（仅计算5s内的数据）
% 计算关节速度 (rad/s)
joint_vel = diff(qs(1:index_limit,:)) / dt1;
...
for i = 1:index_limit
    try
        ...
        warning('正向运动学计算失败在点 %d: %s', i, e.message);
        
        if i > 1 && any(valid_points(1:i-1))
            prev_valid = find(valid_points(1:i-1), 1, 'last');
            tcp_pos(i, :) = tcp_pos(prev_valid, :);
        else
            tcp_pos(i, :) = robot.fkine(robot.qn).t';
        end
    end
end
...
% 关节空间性能评估
stepnorm = vecnorm(diff(qs(1:index_limit,:)),2,2);
...
if ~isempty(tcp_accel_norm)
    fprintf('末端执行器最大加速度: %.4f m/s²\n', max(tcp_accel_norm));
    fprintf('末端执行器平均加速度: %.4f m/s²\n', mean(tcp_accel_norm));
else
    fprintf('末端执行器加速度: 数据不足无法计算\n');
end

% 8. 绘制分析图表
if ~skip_part2 && ~isempty(joint_vel_norm) && ~isempty(joint_accel_norm)
    % 绘制关节速度和加速度曲线
...
if ~skip_part2 && ~isempty(tcp_vel_norm) && ~isempty(tcp_accel_norm)
    % 绘制末端执行器速度和加速度曲线
...
% fprintf('\n===== 系统配置总结 =====\n');
% if skip_part1
%     fprintf('Fuzzy-PP 动画: 跳过 | ');
% else
%     fprintf('Fuzzy-PP 动画: 启用 | ');
% end
% fprintf('速度因子: %.2f\n', speed_part1);
% 
% if skip_part2
%     fprintf('PUMA560 动画: 跳过 | ');
% else
%     fprintf('PUMA560 动画: 启用 | ');
% end
% fprintf('基座高度: %.3f m | 速度因子: %.2f\n', base_height, speed_part2);
...
% % 
% % %% 分别设置两部分动画跳过
% % skip_animation_part1 = input('是否跳过 Fuzzy-PP 动画演示？(1=是, 0=否): ');
% % skip_animation_part2 = input('是否跳过 PUMA560 动画演示？(1=是, 0=否): ');
% % 
% % %% ==================== Part I: Fuzzy‑PP 控制生成笛卡尔轨迹 ====================
% % % 用户参数
% % T1 = 60;        dt1 = 0.02;      N1 = round(T1/dt1);
...
% % fprintf('平均相邻关节位移: %.4f rad\n', max(stepnorm)); % 这里发现原代码有笔误，已修正为mean(stepnorm)
% % fprintf('平均相邻关节位移: %.4f rad\n', mean(stepnorm));
...
% % 集成 Fuzzy‑PP 纯追踪演示 与 PUMA560 关节 PID 控制演示 (跟踪相同路径)
% % MATLAB2022b + Peter Corke Robotics Toolbox 10.4
% %=====================================================================
% clc; clear; close all;
% 
% %% ==================== Part I: Fuzzy‑PP Complex Demo ====================
% % 用户参数
% T1           = 60;        % 仿真时长 (s)
% dt1          = 0.02;      % 时间步长 (s)
% N1           = round(T1/dt1);
...
% x_ref1 = 0.5*cos(phi1) + 0.1*cos(3*phi1);
% y_ref1 = 0.55*sin(phi1) + 0.05*sin(2*phi1);
% 
% % 模糊增益 FIS 自动生成/加载
% fisFile = 'fuzzy_gain.fis';
% if ~isfile(fisFile)
...
% 
% %% ================ Part II: PUMA560 跟踪演示 ================
% % 加载 PUMA560 并初始化
% mdl_puma560; robot = p560;
% q0 = [0,-pi/4,pi/4,0,pi/3,0]; q = q0;
% % 构造与 Fuzzy‑PP 相同的笛卡尔轨迹序列
% steps2 = N1;                      % 步数与 Part I 一致
% targetZ = 0.5;                    % PUMA TCP 高度
% targets = repmat(eye(4),1,1,steps2);
% for i=1:steps2
%     targets(:,:,i) = SE3(xL1(i), yL1(i), targetZ);
% end
% % PID 参数
% Kp = [200,200,150,80,80,30];
% Ki = [20, 20, 10, 5, 5, 2];
% Kd = [80, 80, 40,20,20,10];
% 
% dt2 = dt1; int_e2 = zeros(1,6); prev_e2 = zeros(1,6);
% figure('Name','PUMA560 PID Tracking','NumberTitle','off');
% robot.plot(q0); drawnow;
% % 跟踪
% for i=1:steps2
%     % 逆解，增加容错参数
%     q_des = robot.ikine(targets(:,:,i), q, [1 1 1 0 0 0], 'tol', 1e-3, 'maxiter', 1000);
%     if isempty(q_des)
%         error('逆运动学求解失败于步骤 %d。请检查目标位姿是否可达。', i);
%     end
%     e2 = q_des - q;
%     % PID 控制
%     int_e2 = int_e2 + e2*dt2;
%     d_e2   = (e2 - prev_e2)/dt2;
%     qdot   = Kp.*e2 + Ki.*int_e2 + Kd.*d_e2;
%     q      = q + qdot*dt2;
%     prev_e2= e2;
%     robot.plot(q);
%     drawnow;
% end
% % 关节误差评估
% final_error = abs(q_des - q0);
% rmse_j = sqrt(mean((q_des - q).^2));
% max_j  = max(abs(q_des - q));
% fprintf('\n===== PUMA560 PID 跟踪评估 =====\n');
% fprintf('关节 RMSE = [%.4f %.4f %.4f %.4f %.4f %.4f]\n', rmse_j);
% fprintf('关节最大误差 = [%.4f %.4f %.4f %.4f %.4f %.4f]\n', max_j);
% fprintf('最终关节偏差 = [%.4f %.4f %.4f %.4f %.4f %.4f]\n', final_error);
...
% % 通过 SCALE 将移动机器人路径缩放到 PUMA560 可达工作空间
% % MATLAB2022b + Peter Corke Robotics Toolbox 10.4
% %=====================================================================
% clc; clear; close all;
% 
% %% ==================== Part I: Fuzzy‑PP Complex Demo ====================
% % → 与之前相同，生成移动机器人复杂轨迹，用于演示
% T1 = 60; dt1 = 0.02; N1 = round(T1/dt1);
% V_MAX = 0.6; OMEGA_LIM = 2.5;
% LOOK_BASE = 0.6; SPEED_CUT = 0.3;
% slow_factor = 0.5; refresh_step = 1;
% 
% phi = linspace(0,2*pi,N1);
% % 原始大半径轨迹（半径≈5 m）
...
% 
% % =============================================================
% % Part I 纯追踪动画可选：如需观察移动机器人轨迹，设 enable_pp = true
% % =============================================================
% enable_pp = false;
% if enable_pp
%     % (省略：此处可插入前半部分纯追踪控制器，用于演示和评估)
% end
% 
% %% ==================== Part II: PUMA560 轨迹复现 (闭式解 ikine6s) ====================
% % 使用闭式逆解 ikine6s, 逐点选取最接近前状态的解, 避免 optimoptions 报错
% 
% reach_rad = 0.65;
% max_xy = max(vecnorm([x_ref' y_ref'],2,2));
% SCALE = 0.8*reach_rad/max_xy;
% xs = x_ref*SCALE; ys = y_ref*SCALE; z_tcp = 0.45;
% R_tcp = trotz(pi);  % TCP 姿态固定, Z 轴朝下
% 
% mdl_puma560; robot = p560;
% qs = zeros(N1,6);
% q_prev = zeros(1,6);
...
% unreachable = false;
% for i = 1:N1
%     Tgoal = transl(xs(i),ys(i),z_tcp)*R_tcp;
%     sol = robot.ikine6s(Tgoal);      % 8×6 所有封闭解
%     if isempty(sol)
%         unreachable = true; break;
%     end
%     % 选择与上一姿态最接近的解
%     [~,idx_best] = min(vecnorm(sol - q_prev,2,2));
%     q_best = sol(idx_best,:);
%     qs(i,:) = q_best;
%     q_prev  = q_best;
% end
% 
% if unreachable
%     error('目标路径存在不可达点 — 请调小 SCALE 或抬高 z_tcp');
% end
% 
% % ---------- 动画 ----------
% figure('Name','PUMA560 ikine6s 轨迹','NumberTitle','off');
% robot.plot(qs(1,:),'trail','b.','workspace',[-reach_rad reach_rad -reach_rad reach_rad 0 1]);
% view(135,25); hold on;
% plot3(xs,ys,z_tcp*ones(size(xs)),'k--','LineWidth',1.2);
% for i = 1:N1
%     robot.animate(qs(i,:));
%     drawnow limitrate;
% end
% 
% % ---------- 评估 ----------
% step_norm = vecnorm(diff(qs),2,2);
% fprintf('\n===== PUMA560 ikine6s 轨迹评估 =====\n');
% fprintf('最大相邻步关节范数: %.4f rad\n',max(step_norm));
% fprintf('平均相邻步关节范数: %.4f rad\n',mean(step_norm));
% 
% %% End
...
%=====================================================================
% fuzzy_pp_and_robot_pid_updated.m
% 集成 Fuzzy‑PP 纯追踪演示 与 PUMA560 关节 PID 控制演示
% MATLAB2022b + Peter Corke Robotics Toolbox 10.4
%=====================================================================
% clc; clear; close all;
...
%     fis_g = addMF(fis_g,'g','trimf',[1.4 1.8 2.0],'Name','Large');
%     % 规则表
%     ruleList = [1 1 3 1 1; 1 2 3 1 1; 1 3 2 1 1; 2 1 3 1 1; 2 2 2 1 1; 2 3 1 1 1; 3 1 2 1 1; 3 2 1 1 1; 3 3 1 1 1];
%     fis_g = addRule(fis_g, ruleList);
%     writefis(fis_g, fisFile);
% end
% gain_fis = readfis(fisFile);
...
% state.y = y_ref1(1);
% state.th = atan2(y_ref1(2)-y_ref1(1), x_ref1(2)-x_ref1(1));
% xL1=zeros(1,N1); yL1=zeros(1,N1); eL1=zeros(1,N1); gL1=zeros(1,N1);
% v_hist=zeros(1,5); kappa_hist=zeros(1,5);
% e_lat_integral=0; prev_e=0; e_lat_hist=zeros(1,3);
% 
% % 动画准备
% win=1.5;
...
%     dists=hypot(x_ref1-state.x,y_ref1-state.y);
%     [~,idx]=min(dists);
%     dx=x_ref1(idx)-state.x; dy=y_ref1(idx)-state.y;
%     e_lat=sin(state.th)*dx - cos(state.th)*dy;
%     psi_err=wrapToPi(atan2(dy,dx)-state.th);
%     % 误差率与历史
%     e_lat_rate=(e_lat-prev_e)/dt1;
...
%         s=(A+B+C)/2; area=sqrt(max(s*(s-A)*(s-B)*(s-C),0));
%         if area>1e-6
%             kappa=4*area/(A*B*C);
%         else
%             kappa=0;
...
%     psi_n=max(min(psi_err/pi,1),-1);
%     gamma=evalfis([e_n,psi_n],gain_fis);
%     if abs(kappa)>0.2, gamma=gamma*(1+0.6*min(abs(kappa)/10,1)); end
%     if abs(e_lat)>0.5||abs(e_lat_integral)>1||abs(e_lat_rate)>0.5, gamma=gamma*2.5; end
%     omega=max(min(gamma*omega_pp,OMEGA_LIM),-OMEGA_LIM);
%     % 自适应速度
%     speed_f=1-0.9*min(abs(kappa)/10,1);
%     err_f2=1-0.6*min(abs(e_lat)/SPEED_CUT,1);
...
%     state.y=state.y+v*sin(state.th)*dt1;
%     state.th=wrapToPi(state.th+omega*dt1);
%     xL1(k)=state.x; yL1(k)=state.y; eL1(k)=abs(e_lat); gL1(k)=gamma;
%     % 动画
%     addpoints(anim1,state.x,state.y);
%     if mod(k,refresh_step)==0
%         set(errTxt,'Position',[state.x,state.y+0.3],'String',sprintf('e=%.3f m',eL1(k)));
%         xlim([state.x-win,state.x+win]); ylim([state.y-win,state.y+win]);
%         drawnow limitrate; pause(dt1*slow_factor);
%     end
% end
% % 性能评估
% RMSE1=sqrt(mean(eL1.^2)); MAXE1=max(eL1);
% SS1=mean(eL1(end-round(2/dt1):end)); INT1=sum(eL1)*dt1;
% fprintf('\n===== Fuzzy‑PP Complex Demo 评估 =====\n');
% fprintf('RMSE   = %.4f m (目标 ≤ 0.05 m)\n',RMSE1);
% fprintf('Max E  = %.4f m\n',MAXE1);
% fprintf('SS Err = %.4f m\n',SS1);
% fprintf('Int E  = %.4f m·s\n',INT1);
% % 静态结果
% figure('Name','Fuzzy‑PP Static Results');
% subplot(2,2,1); plot(x_ref1,y_ref1,'k--',xL1,yL1,'b','LineWidth',1.5); axis equal; grid on; title('轨迹对比');
% subplot(2,2,2); plot((0:N1-1)*dt1,eL1,'LineWidth',1.2); grid on; ylabel('e (m)'); title('距离误差');
% subplot(2,2,3); plot((0:N1-1)*dt1,gL1,'LineWidth',1.2); grid on; ylabel('γ'); title('模糊增益'); xlabel('t (s)');
% subplot(2,2,4); histogram(eL1,30); title('误差分布');
% 
% %% ================= Part II: Industrial Robot PID Demo =================
% % 加载 PUMA560 模型
% mdl_puma560;
% robot=p560;
% % 初始关节角
% q0=[0,-pi/4,pi/4,0,pi/3,0]; q=q0;
% % 生成复杂笛卡尔轨迹
% steps2=20; theta2=linspace(0,2*pi,steps2);
% r2=0.15; cx=0.5; cy=0; cz0=0.2; amp_z=0.05;
% traj_cart(steps2)=SE3();
% for i=1:steps2
%     x=cx+r2*cos(theta2(i)); y=cy+r2*sin(theta2(i)); z=cz0+amp_z*sin(2*theta2(i));
%     T=SE3(x,y,z)*SE3.Rz(theta2(i)); traj_cart(i)=T;
% end
% % 逆解期望轨迹
% q_des=zeros(steps2,6); q_prev=q0; mask=[1 1 1 1 1 1];
% for i=1:steps2
%     q_prev=robot.ikine(traj_cart(i),q_prev,mask); q_des(i,:)=q_prev;
% end
% % PID 参数
% Kp=[200,200,150,80,80,30]; Ki=[20,20,10,5,5,2]; Kd=[80,80,40,20,20,10];
% dt2=0.05; int_e2=zeros(1,6); prev_e2=zeros(1,6);
% figure('Name','PUMA560 PID Tracking','NumberTitle','off'); robot.plot(q0); drawnow;
% for i=1:steps2
%     e2=q_des(i,:)-q; int_e2=int_e2+e2*dt2; d_e2=(e2-prev_e2)/dt2;
%     qdot=Kp.*e2+Ki.*int_e2+Kd.*d_e2; q=q+qdot*dt2; prev_e2=e2;
%     robot.plot(q); title(sprintf('Step %d/%d',i,steps2)); drawnow;
% end
% fprintf('\n===== Industrial Robot PID Demo 完成 =====\n');
...
% figure('Name','Fuzzy‑PP Static Results');
% subplot(2,2,1);
% plot(x_ref1,y_ref1,'k--',xL1,yL1,'b','LineWidth',1.5); axis equal; grid on; title('轨迹对比');
% subplot(2,2,2);
% plot((0:N1-1)*dt1,eL1,'LineWidth',1.2); grid on; ylabel('e (m)'); title('距离误差');
% subplot(2,2,3);
% plot((0:N1-1)*dt1,gL1,'LineWidth',1.2); grid on; ylabel('γ'); title('模糊增益'); xlabel('t (s)');
% subplot(2,2,4);
% histogram(eL1,30); title('误差分布');
% 
% %% ================= Part II: Industrial Robot PID Demo =================
% % 加载 PUMA560 模型
% mdl_puma560;
...
% 
% % 初始关节角
% q0 = [0, -pi/4, pi/4, 0, pi/3, 0];
% q  = q0;
% 
% % 生成复杂笛卡尔轨迹
% steps2 = 200;
% theta2 = linspace(0,2*pi,steps2);
% r2     = 0.15; cx=0.5; cy=0; cz0=0.2; amp_z=0.05;
% traj_cart(steps2) = SE3();
% for i=1:steps2
%     x = cx + r2*cos(theta2(i));
%     y = cy + r2*sin(theta2(i));
%     z = cz0 + amp_z*sin(2*theta2(i));
%     T = transl(x,y,z)*SE3.Rz(theta2(i));
%     traj_cart(i) = T;
% end
% 
% % 逆解得到期望关节轨迹
% q_des = zeros(steps2,6);
% q_prev = q0; mask=[1 1 1 1 1 1];
% for i=1:steps2
%     q_prev = robot.ikine(traj_cart(i), q_prev, mask);
%     q_des(i,:) = q_prev;
% end
% 
% % PID 参数设置
...
% 
% dt2 = 0.05;
% int_e2 = zeros(1,6); prev_e2 = zeros(1,6);
% 
% figure('Name','PUMA560 PID Tracking','NumberTitle','off');
% robot.plot(q0); drawnow;
% for i=1:steps2
...
%     prev_e2 = e2;
%     robot.plot(q);
%     title(sprintf('Step %d / %d', i,steps2));
%     drawnow;
% end
% 
% fprintf('\n===== Industrial Robot PID Demo 完成 =====\n');
...
% %=====================================================================
% % industrial_robot_complex_pid.m
% % MATLAB2022b + Peter Corke Robotics Toolbox 10.4
% % 工业机器人复杂轨迹＋PID 闭环仿真
% %=====================================================================
% clear; clc; close all;
% 
% %% 1. 加载机器人模型 (PUMA 560)
% mdl_puma560;           % 从 Toolbox 加载 p560
% robot = p560;
% 
% %% 2. 初始关节配置
% q0 = [0, -pi/4, pi/4, 0, pi/3, 0];
% 
% %% 3. 生成复杂笛卡尔轨迹 (circle + z 波动 + 姿态旋转)
% steps = 200;                            % 总步数
% theta = linspace(0, 2*pi, steps);      
% r = 0.15;                               % 圆半径
% cx = 0.5; cy = 0.0; cz0 = 0.2;          % 圆心偏移
% amp_z = 0.05;                           % z 方向波动幅度
% 
% % 预分配 SE3 数组
% traj_cart(steps) = SE3();             
...
%     x = cx + r*cos(theta(i));
%     y = cy + r*sin(theta(i));
%     z = cz0 + amp_z*sin(2*theta(i));
%     % 位置＋末端绕 Z 旋转
%     T = transl(x, y, z) * SE3.Rz(theta(i));
%     traj_cart(i) = T;  % 确保是 SE3 对象
% end
% 
% %% 4. 逆运动学求解关节期望值
% q_des = zeros(steps, 6);
% q_prev = q0;
% mask  = [1 1 1 1 1 1];   % 全自由度
% for i = 1:steps
%     q_prev = robot.ikine(traj_cart(i), q_prev, mask);
%     q_des(i, :) = q_prev;
% end
...
% prev_e = zeros(1,6);
% 
% figure('Name','复杂轨迹 PID 仿真','NumberTitle','off');
% robot.plot(q0);         % 初始显示
% 
% for i = 1:steps
%     % 6.1 计算误差
...
%     % 6.4 绘制
%     robot.plot(q);
%     title(sprintf('Step %d / %d', i, steps));
%     drawnow;
% end
