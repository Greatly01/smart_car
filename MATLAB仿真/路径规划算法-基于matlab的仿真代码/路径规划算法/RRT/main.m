...
goal = [90, 93];   % goal position
n = 5;             % no. of obstacles:障碍物数量

%% Defining the grid:定义网格

% % %%%% 1st Config %%%%%%
% figure
% rectangle('Position',[20 10 40 30], 'FaceColor',[1 0 0], 'EdgeColor',[1 0 0])
% % hold on
% rectangle('Position',[50 60 20 20], 'FaceColor',[1 0 0], 'EdgeColor',[1 0 0])
% %%%%%%%%%%%%%%%%%%%%%
% axis([0 100 0 100])
% box on;
% axis square;
...
% %%%%%%%%%%%%%
% 
% Plotting start position;绘制起始位置
circles(start(1), start(2),2, 'facecolor','green')

% Plotting goal position：绘制目标位置
circles(goal(1), goal(2),2, 'facecolor','yellow')

% initialising the hash map：初始化哈希映射
%%% 1st Config
...
values = {start, [10,5], [10,20], [40,25], [40,20], [50,5], [50,25], [65,25], [65,5], [70,10], [70,30], [90,30], [90,10], [10,40], [10,60], [30,60], [30,40], [40,40], [40,60], [75,60], [75,40], [80,35], [80,75], [95,75], [95,35], [10,70], [10,90], [15,90], [15,70], [20,70], [20,90], [40,90], [40,70], [50,65], [50,90], [70,90], [70,65], goal};
%%%%%%%%%%%%%%%%%
...
% Building all the obstacle edges:构建所有障碍物边缘
length = size(keys);    % this contains the number of nodes:包含节点数
...
% Removing edges which are not obstacle edges:移除非障碍物边缘
...
% 在每个障碍物中添加 1 个边对，这些边对在前面的障碍物中没有添加
% for loop
i = 2;
while i < length(2)-2    
...
%% RRT Algorithm:RRT 算法
G = graph;                      % initialising the graph
step = 1;                       % step size for the RRT algo
...
%% find the shortest path from the graph generated from RRT:规划最优路径
