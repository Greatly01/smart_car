function rrts = rrt
rrts.angel_dist = @angel_dist;                            % Calculate the distance between two points
...
rrts.angel_limit=@angel_limit;                            % Calculate the angle between two vectors
rrts.ccw=@ccw                                             % Determine which side of the line the point is on
rrts.noCollision=@noCollision                             % Determine whether there is a collision with an obstacle
end

function val = ccw(A,B,C)
    val = (C(2)-A(2)) * (A(1)-B(1)) > (A(2)-B(2)) * (C(1)-A(1));
end
